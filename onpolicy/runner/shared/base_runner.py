...
class Runner(object):
    ...
    def restore(self):
        ...
        if not self.all_args.use_render:
            ...
            new_policy_state_dict = OrderedDict()
            s ="v_out.stddev , v_out.weight , v_out.bias v_out.mean , v_out.mean_sq , v_out.debiasing_term"

            for key,value in policy_critic_state_dict.items():
                if key in s:
                    1
                else:
                    new_policy_state_dict[key]=value

           # self.policy.critic.load_state_dict(policy_critic_state_dict)
            self.policy.critic.load_state_dict(new_policy_state_dict)
            # self.policy.critic.load_state_dict(policy_critic_state_dict)
