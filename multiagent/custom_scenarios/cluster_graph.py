...
from multiagent.core import Satellite, Landmark, Entity,  SatWorld
...
class SatelliteScenario(BaseScenario):
    ...
    def make_world(self, args: argparse.Namespace) -> SatWorld:
        ...
        world.scripted_agents = [Satellite() for _ in range(self.num_scripted_agents)]
    ...
    def random_scenario(self, world):
        ...
        while True:
            ...
            if not obs_collision and not agent_collision:
                ...
                world.scripted_agents[num_scripted_agents_added].state.p_vel =  (1/1000)*np.ones(world.dim_p)
        ...
        landmark_pose = 0.8 * np.random.uniform(-self.world_size / 2, self.world_size / 2, world.dim_p )
        ...
        for i in range(self.num_agents):
            world.landmarks[num_goals_added].state.p_pos = expected_poses[i]
    ...
    def is_collision(self, agent1: Satellite, agent2: Satellite) -> bool:
    ...
    def observation(self, agent: Satellite, world: SatWorld,  local_obs:bool=None) -> arr:
        """
        Return:
    ...
    def get_id(self, agent: Satellite) -> arr:
        return np.array([agent.global_id])

    def graph_observation(self, agent: Satellite, world: SatWorld) -> Tuple[arr, arr]:
        """
    ...
    def _get_entity_feat_relative(
        self, agent: Satellite, entity: Entity, world: SatWorld
    ) -> arr:
        """
        Returns: ([velocity, position, goal_pos, entity_type])
...
if __name__ == "__main__":
    ...
    env = SatelliteMultiAgentGraphEnv(
        world=world,
